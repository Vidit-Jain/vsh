				...
				if (inputCharacter == 27) {
					...
					if (read(STDIN_FILENO, buffer, 2) == 2) {
						...
						if (buffer[0] == '[' && buffer[1] == 'A') {
							upArrow(input);
						} else if (buffer[0] == '[' && buffer[1] == 'B') { // 'B' indicates it will be a down arrow key
							downArrow(input);
						}
					}
				} else if (inputCharacter == 127) { // 127 - Backspace key
					if (input->length > 0) {
						if (input->str[input->length - 1] == 9) { // Handle tabs separately
							for (int i = 0; i < 7; i++)
								printf("\b");
						}
					...
					exit(0);
				} else { // Unhandled control sequence, can just print, don't store in input
					printf("%c\n", inputCharacter);
				}
			} else {
